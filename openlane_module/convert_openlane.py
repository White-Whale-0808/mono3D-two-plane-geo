...
1. **GT 座標**：OpenLane 的 ``lane_lines[i]['xyz']`` 已經在相機光學座標系
   （x 前、y 左、z 上，原點在相機）。**判別實驗**（2026-09-11，12 段 479 條線，
   比「標註 uv 到投影折線的距離」而非逐索引，因為 uv 與 xyz 不逐點對應）：

       假說 A  xyz 已在光學系，只用內參     -> 殘差中位  0.90 px
       假說 B  xyz 在車體系，先套外參逆旋轉 -> 殘差中位 11.75 px   (A 好 13 倍)

   旁證：xyz 的高度中位數 −2.13 m，正是相機掛載高度（車體系原點在接地點的話
   應該落在 0 附近）。
   ⚠ 先前這裡寫的理由是「extrinsic 旋轉 ≈ 單位矩陣（掛載 pitch 僅 0.17°）」，
   **兩處都不對**：實測總旋轉角中位 0.705°（roll 0.44° 最大、pitch 0.25°、
   yaw 0.17° —— 0.17 是 yaw 不是 pitch），而且「旋轉很小所以兩個座標系差不多」
   本來就是錯的推論方向 —— 0.7° 在 60 m 處是 0.7 m，比要量的起伏還大。
   要分辨座標系得用上面的判別，不能靠「夠接近」。

   :class:`RoadProfileGT` 算的是 ``v = P_world − cam_world`` 再投影到相機基底，
   所以只要把相機位姿寫成**單位姿態、原點在 0**，世界座標就等於相機座標，
   ``z_gt = x``、``h_gt = z``，不需要任何 offset 或弧長換算。
...
# ---------------------------------------------------------------- 篩選門檻
# 在這個深度決定「哪一條是內側線」。可辯護的區間是 10–15 m，12 在覆蓋率的膝點
# （2026-09-11 掃 50 段 9,874 幀，每幀在 8/10/12/15/20/25 m 各跑一次）：
#   下界＝覆蓋率。標註最近距離中位數 8.06 m，probe 壓在 8 m 只找得到 1,724 幀，
#     不到 12 m（3,490）的一半；10 m 也還少 12%。
#   上界＝挑錯線。20 m 有 10%、25 m 有 18% 的幀挑到「不同的一對」——遠處車道在
#     影像上收斂，過彎後橫向最內側的線可能已經屬於隔壁車道。15 m 覆蓋率略高
#     （3,543）但只差 1.5%，而近處「我這條車道」的語意更可靠，所以取近的。
