...
**採集密度已改**：`_PROFILE_STEP_M` 1.0 → **0.125 m**，對齊 legacy GT 的密度
（18 km/h @ 40 fps，相機每幀前進 0.1246 m）。原因：GT 的 pitch 是 `np.gradient`
逐點微分，1 m 間距等於把剖面當折線。**試過離線補救**（把所有幀的剖面點在世界
座標下合併，可到 1 cm）**但更差** —— 單幀那批點是同一瞬間、同一相機姿態打出來
的，內部一致；跨幀合併會混入配準誤差（用里程對齊更糟，7.7 mm 假散布）。
密度要在採集時給。代價是每幀 `next()` 與 `cast_ray` 從 51 次變 401 次，
只花牆鐘時間。可用 `--profile-step-m` 調整。
