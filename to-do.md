...
- [ ] **直接驗證（不需重跑 CARLA）**：`verify_carla_geometry.py` 那趟同時記了
      `h_wp_cam`（相機到 waypoint）與 `h_ray`（相機射線打到**實際網格**），
      400 幀都有值。兩者相減就是「解析曲線 − 實際網格」。
      資料在 `outputs/carla_geometry_verification_20260730_235516.json`
- [ ] **若證實**：下次採集的前方剖面改用 `world.cast_ray()` 打網格（第 1 組已
      驗證可用，記得用 label 過濾避免打到引擎蓋），或兩種都存
- [ ] 評估這條對現有結論的影響。`w_real` 的判定是用**高度殘差**做的，網格偏差
      若是零均值就不影響尺度，但會墊高 MAE，也可能污染振幅缺口的量測

> 但書：比值 1.53、只有 26 個箱，是**有指向性的證據不是定論**；殘差裡也混著
> pipeline 自己的誤差。定案要看上面第一項的直接比較。
