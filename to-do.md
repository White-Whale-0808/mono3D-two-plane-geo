...
> 「已完成 / 已失效」的紀錄以免重複討論。
>
> **2026-08-18 更新**：第 2/3 組**已完成**（新資料集
> `carla_dataset_Town03_20260818_195221`，495 幀，含前方路面剖面與相機完整
> transform）。用量到的 GT 重新評估後，**`w_real` 已改為物理值 3.25**。
> 過程中被推翻的假說見「第 1 組的後續」。行號未重新對照。
...
當時推測誤差來源是「`collect_dist_m` 是 3D 弧長、pipeline 的 z 是深度」。
**該假說已於 2026-08-18 離線推翻，改寫見下面兩節。**
...
### 弧長假說：**已推翻**（2026-08-18，純離線）

工具在 `debug/`（不進版控）：`dump_pitch_curves.py` 把每幀的
`(z_samples, pitch_samples)` 存成 npz、`gt_axis.py` 提供兩種 GT 距離軸、
`eval_gt_axis.py` 做 2×2 與尺度掃描。
**要點：改 GT 軸完全不動 pipeline 輸出**，所以一次 dump 就能精確評估所有軸
變體，不必為每個假設重跑 PIDNet+ELSED。

**正確的弧長→深度轉換不是 `z·cos(θ)`**，而是

    x_i(s) = ∫ cos(θ(σ) − p_i) dσ

關鍵是 `− p_i`：**相機剛性固定在車上，車在坡上時光軸跟著坡一起傾斜**，
所以**等坡度路段的修正量是零**，只有相機與目標點之間的坡度**變化**才動到軸。
本資料集實測壓縮量僅 0.03%~0.99%。

驗證：`arc` 軸逐位重現基準 0.2331 / 0.1845 / 0.4291 / 0.7223（n=451）。

| GT 軸 | mean | median |
|---|---|---|
| `arc`（現行） | **0.2331** | 0.1845 |
| `depth`（弧長修正後） | **0.2383** | 0.1938 |

修正後**更差**，且把最佳尺度推得更遠離 3.25 → **方向錯、量級也不夠**。
（混淆已排除：所有 scale 下有效樣本固定 90200、451 幀零 NaN。）

`collect_dist_m` 本身沒問題 —— 它是同步模式下逐幀 3D 位移累加
（`get_carlaDataset.py` L425），是真實路徑長度，無積分尺度誤差。

### 真正的缺口比原本以為的大

尺度掃描（同一組樣本）的最佳點在 **`w_real` ≈ 3.10（mean 0.2010）**，
物理值 3.25 給 0.2532。**pipeline 深度被系統性高估約 3.5%**，不是原本
爭論的 1%，而且方向是**低於**物理值。

二維掃描（乘性尺度 × 加性 offset）另外確認：`camera_forward_offset = 1.5`
是**尖銳最佳值**（改 0 → 0.4326、改 2.5 → 0.5111），WWH-11 的對齊修正再次獲證。

殘差結構（pooled n=90200）：**`pred = 0.9675 × GT + 0.12°`** ——
pipeline 的 pitch 一律比 GT 平約 3.3%，是**角度成比例偏小**，不是距離對不齊。

### 已證實的首要來源：GT 記的是車身姿態，不是路面

`verify_carla_geometry.py` **早就逐幀同時記了** `body_pitch_deg` 與
`road_pitch_deg`，所以這題不必再跑遠端。
`outputs/carla_geometry_verification_20260730_235516.json` 的 400 幀 driving
（路面 0→12.42°）給出

    body = 1.0213 × road − 0.4801°      (corr 0.996)

- 車身振幅超出 **+2.13%**，殘差需要 3.36% → **吃掉約 2/3 的缺口**
- −0.48° 常數偏移在相對剖面裡抵消，不影響
- 逐帶差值 −0.18/−0.51/−0.64/−0.45/−0.10/−0.21 **不是常數**，坡度變化中的
  路段差最多 → 是**懸吊的遲滯**，純比例模型抓不到。
  **這正是第 3 組要解的東西**：`waypoint.next(d)` 直接讀路面，
  完全不經過車身與懸吊，連遲滯一起消掉

把 GT 除以 1.0213 的估計效果（直接算在 dump 上）：

| GT | `w_real`=3.216 | 最佳 `w_real` | 最佳 MAE |
|---|---|---|---|
| 車身 pitch（現行） | 0.2331 | 3.103 | 0.2010 |
| 路面 pitch（估計） | **0.2018** | **3.152** | 0.1943 |

→ **第 2/3 組重採值得做**（同一 `w_real` 下 MAE −13.4%，最佳尺度朝物理值
移動），**但補不滿**：最佳點仍是 ~3.15 而非 3.25，還有約 1% 沒有著落。

保留：1.0213 來自驗證那趟（條件對齊但非同一批幀），且把關係當成純振幅、
未納入遲滯，所以 0.2018 是估計不是量測。

### ✅ 已完成（2026-08-18 深夜）：物理值 3.25 由量到的 GT 證實

新資料集把 GT 從「重建」換成「量測」之後，**四條獨立的幾何路徑全部收斂到
3.243~3.250**：

| 方法 | `w_real` |
|---|---|
| 高度殘差最小化（`Y_3d(z)` vs `h_gt(z)`，不需微分） | 3.248 |
| 由平均高度殘差反推 −H·δ | 3.246 |
| 隱含車道寬（正向投影，2-15 m） | 3.243 ~ 3.260 |
| `cy = 256` 約束（理想針孔，主點在中心） | 3.2432 |
| 物理幾何（地圖 + 漆緣） | 3.25 |

**config 已改為 `w_real: 3.25`，且 `input` / `csv_io` 已指向新資料集。**

> ⚠ **3.25 綁定這條路的標線組合**：本資料集是**左雙黃、右單白**。`w_real` 是內緣到
> 內緣，兩側從 3.5 m 的邊界中心寬各內縮不同的量（左 0.1875、右 0.0625，合計 0.25）。
> 換標線就要重推：同樣 3.5 m 車道下，單+單 = 3.375、雙+單 = 3.25（本資料集）、
> 雙+雙 = 3.125；換 `lane_width` 又另計。**不要把 3.25 帶到別的地圖或別條車道。**

#### 過程中被推翻的兩個假說（記錄以免重蹈）

| 假說 | 怎麼死的 |
|---|---|
| 弧長 vs 深度 | 方向相反、量級只有 0.03~0.99%（相機剛性固定在車上，光軸跟坡一起翹，等坡度修正量為零） |
| 車身/路面振幅比 1.0213 | `road_pitch` 記在相機處、`body_pitch` 記在車輛原點，差 1.5 m；空間對齊後兩趟都是 **0.97**，方向相反 |

另外三個關於「pipeline 把坡度估平 3%」的假說也都被推翻：估測器平滑
（point/windowed 兩種 GT 給相同的 a）、迴歸衰減（**視窗加大 a 沒趨近 1**）、
迴歸稀釋（正反向擬合只差 0.5%）。**真相是彙總權重扭曲** —— 逐幀比較的
振幅比是 0.985~0.995，缺口只有 0.5~1.5%，不是 3%。

**教訓：不要把所有取樣點倒在一起做迴歸**，不同幀的 pitch 範圍與取樣密度不同，
pooled 迴歸會做出假的系統性偏差。本次三個錯誤結論（0.9617、1.0213、3% 缺口）
全是同一類。

#### ⚠ 評估方式的重要結論

**不要用 profile MAE 去定 `w_real`。** MAE 會拿 z 尺度去補償 pitch 的振幅誤差
（上坡剖面上把 z 縮小 = 讀更近更平的 GT），最佳點被系統性拉低 ——
實測 MAE 掃描偏好 3.12~3.15，而幾何說 3.25。
**`w_real` 用高度/幾何定，MAE 只用來報告精度。**

#### 殘留

- pitch 振幅仍低 **0.5~1.5%**；等價地，反解的 `cy` 有 **+0.031 px/m** 的深度漂移
  （跨 35 m 漂 1.08 px）。目前唯一未解的項目，量級很小
- 兩支診斷（反解深度、正向投影）在 **z>15~20 m 會發散**，是地平線附近病態，
  遠段目前沒有獨立驗證

### 舊的驗收條件（2026-08-18 稍早改寫，現已完成）

**舊條件「把 `w_real` 改成 3.25，預期 MAE 低於 0.2331」已作廢** —— 它會
**假性通過**：修好 GT 後 3.25 給 0.2151，確實低於 0.2331，但同一份 GT 下
3.216 給 0.2018 仍然更好。舊條件拿「新 GT 的 3.25」比「舊 GT 的 3.216」，
基準不對等，會誤判成「物理值終於勝出」。

改成**同一份 GT 下比較不同 `w_real`**：

- [ ] 重採後（有 `road_pitch_deg` / 前方剖面）重建 GT，**先固定 `w_real`=3.216
      重跑 batch**，確認 MAE 確實掉到 0.20 附近（驗證 1.0213 的估計）
- [ ] 在**同一份新 GT** 上掃 `w_real`，記錄最佳點落在哪裡
- [ ] 驗收 = **最佳點是否收斂到物理值 3.25**（不是「3.25 有沒有比舊基準好」）
- [ ] 若最佳點停在 3.15 附近（如目前估計），表示還有約 1% 的尺度誤差未解，
      要回頭查 pipeline 側（估測器、`f_x`、`cy` 的車身俯仰假象）
...
> **採集器已實作（2026-08-18），待在遠端實跑。** 開發機沒有 carla，
> 只做過 `py_compile` + stub 模組離線測純邏輯。

- [x] **路面坡度（與車身姿態分開）** ← **本組最高優先，效益已量化（2026-08-18）**
      目前 `gt_pitch_deg` 記的是 `transform.rotation.pitch` = **車身姿態**（含懸吊），
      但 pipeline 量的是**路面**。實測 `body = 1.0213 × road − 0.48°`，
      振幅超出 2.13%，佔目前殘差缺口的約 2/3；估計效益是同一 `w_real` 下
      **MAE 0.2331 → 0.2018（−13.4%）**。加一欄 `road_pitch_deg`：
      ```python
...
- [x] **車輛世界座標** —— 實作時一併加了**相機的完整 transform**（位置 + 旋轉）：
      pipeline 的 z 是沿光軸的距離，光軸方向由相機旋轉定義，有了它「弧長 vs
      深度」以後都是離線算術而非假說
      目前只存累積距離。有了 (x,y,z) 就能：
      (a) ~~分辨**弧長 vs 水平距離**~~ —— **此動機已作廢（2026-08-18）**：弧長效應
          實測只有 0.03~0.99% 且方向相反（見上面「弧長假說：已推翻」）。
          `collect_dist_m` 是逐幀 3D 位移累加，本身沒有尺度誤差
...
- [x] **直接採樣前方路面剖面，取代「用行駛歷史回推」** —— `sample_road_profile`，
      **存 waypoint 的世界座標而非預先算好的 pitch**（存 pitch 會把座標系的選擇
      燒死在資料裡，選錯要重採）。逐步走並在路口選航向最接近的分支
      現在 `gt_pitch_profile` 是拿「車子後來開到那裡時的 pitch」當作前方 GT
      （`pitch_visualization.py` 模組 docstring）。這帶來三個問題：
      **車身/懸吊的遲滯**（實測逐坡度帶的 body−road 差值不是常數，坡度變化中的
      路段差最多 −0.64°，純比例修正抓不到 —— 直接讀路面才能消掉）、
      前移 1.5 m 的對齊（實測 1.5 是尖銳最佳值，這項目前是對的）、
      以及**開到終點的幀沒有前方 GT**。
      改成採集當下直接問地圖：
      ```python
