...
**階段 C 已完成（2026-09-18）**：近場量測是車道寬的唯一來源，`nearfield_w_real`
開關與 `--nearfield` 已刪，pipeline 不再讀 config 的 `w_real`。
剩下三步，**依序**做：
...
- [x] ~~**1. 局部性 fallback**~~ 由使用者裁定取代（2026-09-18）：三層。
      連續片段內沿用上次量到的值（不限距離，沿用多久寫進 CSV）；片段內從沒量到
      過才用 `last_resort_lane_width`（3.25，原 `w_real` 改名）並標 `last_resort`。
      實跑（無最後手段時，逐幀：量到 / 沿用 / 無錨；無錨即現在的 last_resort 幀）：
      CARLA 三路線 2,421 幀 1388 / 808 / 225，pitch 輸出 1924；OpenLane Tier A
      488 幀 340 / 69 / 79，輸出 381。無錨主因是近場取不到列（CARLA 173/225）；
      down_hile 開頭 21 m（174 幀）全無輸出。浮現兩個新問題：
      - [x] **採用門檻餓死閃爍片段**：OpenLane 17612 近場每隔一幀取不到列，整段
        30 幀無輸出 → 已修：run 容許中斷一幀（2.99 vs 真值 2.88）。「z_hi 內任一
        通過即可」試過並否決（CARLA 寬度 p90 6.2 → 10.1%，放進轉折點的值）
      - [ ] **沿用值在路寬變化時過期**：OpenLane 13356 路寬 22 m 內 3.07 → 3.79，
        只在第 1 幀量到 2.71（−13%），之後通過品質閘門的只有 3 幀且彼此隔 ≥2 幀
        失敗，所以一路沿用到 −29%。採用規則修不了（好的那幾幀是被品質閘門擋掉）
